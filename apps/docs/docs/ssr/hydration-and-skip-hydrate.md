...
Both Next.js adapters rely on the same three functions from `nekuta` (not `@devtools/next` — this logic is router-agnostic, the adapters only handle _transporting_ the result):
...
- **`hydrateNekutaState(nekuta, serialized)`** — merges a deserialized state tree into a `Nekuta` instance. Must run **before** any store owning one of those ids is first resolved, so its `state()`/setup function sees the hydrated values instead of its own defaults — both `@devtools/next` providers already call this at the right time; you only need it directly if you're building a custom hydration flow.
...
```ts
import { skipHydrate } from '@devtools/core';

...
function getTagVersionFromNpm(tag) {
    try {
        return child_process
            .execSync(`npm info ${packageJSON.name} version --tag="${tag}"`)
            .toString('utf8')
            .trim();
    } catch (e) {
        throw e;
...
function desiredTargetVersion(ref) {
    ...
    if (results != null) {
        if (results[1] !== tagArgument) {
            console.warn(
                `The base branch name (${results[1]}) differs from the tag name ${tagArgument}`
            );
        }
    ...
    throw new Error(
        'Malformed branch name for ref: ' +
            ref +
            ". Can't derive the base version. Use a branch name like: beta-x.x.x!"
    );
}
...
let publishTag;
if (semver.eq(baseVersion, latestReleaseBase)) {
    // check if we are releasing another version for the latest beta
